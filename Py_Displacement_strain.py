...
# 1) A function that computes spherical harmonic derivative
#    with respect to colatitude (theta) and longitude (phi)
#    on the fly
# 2) A function that computes spherical harmonic derivative
#    and store them in a file
# 3) A function that computes the Banerdt et al. (1986) equations
#    to determine strains from displacements with a correction
#    to the theta_phi term
# 4) A function that computes principal strains and angles
    ...
    for l in range(lmax+1):
         ...
         for m in range(-l,l+1):
             ...
             if m >= 0:
                 ...
                 Y_lm_d2_thetaphi_a[0,l,m] = dp_theta[index] * \
                 msinmphi
             else:
                 mcosmphi = m_abs * cosmphi
                 m2sinphi = -m_abs**2 * sinmphi
                 Y_lm_d1_theta_a[1,l,m_abs] = dp_theta[index] * \
                 sinmphi
                 Y_lm_d1_phi_a[1,l,m_abs] = p_theta[index] * \
                 mcosmphi
                 Y_lm_d2_phi_a[1,l,m_abs] = p_theta[index] * \
                 m2sinphi
                 Y_lm_d2_thetaphi_a[1,l,m_abs] = \
                 dp_theta[index] * mcosmphi
         
         if theta == 0 or theta == pi:
             Y_lm_d2_theta_a[:,l,:l+1] = 0. # Not defined.
         else:
             # Make use of the Laplacian identity to estimate
             # last derivative.
             Y_lm_d2_theta_a[:,l,:l+1] = lapla * \
             y_lm[:,l,:l+1] - Y_lm_d1_theta_a[:,l,:l+1] * \
             costsint - sintt * Y_lm_d2_phi_a[:,l,:l+1]
                    
    return Y_lm_d1_theta_a, Y_lm_d1_phi_a, Y_lm_d2_theta_a, \
    Y_lm_d2_phi_a, Y_lm_d2_thetaphi_a, y_lm
...
def SH_deriv_theta_phi(lmax, path):
    ...
    if Path(poly_file).exists()==0:
        ...
        phi_ar = np.linspace(0,360,2*n,endpoint=False)*pi/180.
        
        t_i = -1
        for theta in np.linspace(0,180,n,endpoint=False)*pi/180.:
            ...
            for l in range(lmax+1):
                ...
                for m in range(-l,l+1):
                    ...
                    if m >= 0:
                        ...
                        m2cosphi = -m**2 * cosmphi # Second
                                                   # cos(m*phi)
                                                   # derivative
                        Y_lm_d1_theta_a[t_i,:,0,l,m] = \
                        dp_theta[index] * cosmphi
                        Y_lm_d1_phi_a[t_i,:,0,l,m] = \
                        p_theta[index] * msinmphi
                        Y_lm_d2_phi_a[t_i,:,0,l,m] = \
                        p_theta[index] * m2cosphi
                        Y_lm_d2_thetaphi_a[t_i,:,0,l,m] = \
                        dp_theta[index] * msinmphi
                    else:
                        mcosmphi = m_abs * cosmphi
                        m2sinphi = -m_abs**2 * sinmphi
                        Y_lm_d1_theta_a[t_i,:,1,l,m_abs] = \
                        dp_theta[index] * sinmphi
                        Y_lm_d1_phi_a[t_i,:,1,l,m_abs] = \
                        p_theta[index] * mcosmphi
                        Y_lm_d2_phi_a[t_i,:,1,l,m_abs] = \
                        p_theta[index] * m2sinphi
                ...
                for phi in phi_ar:
                    p_i += 1
                    y_lm = pysh.expand.spharm(l, theta, phi, \
                    degrees = False)
                    if theta == 0 or theta == pi:
                        Y_lm_d2_theta_a[t_i,p_i,:,l,:l+1] = 0.
                        # Not defined.
                    else:
                       ...
                       # estimate last derivative.
                        ...
                        lapla * y_lm[:,l,:l+1] - \
                        Y_lm_d1_theta_a[t_i,p_i,:,l,:l+1] * \
                        costsint - sintt * \
                        Y_lm_d2_phi_a[t_i,p_i,:,l,:l+1]
                    
        np.save(poly_file, [Y_lm_d1_theta_a, Y_lm_d1_phi_a, \
        Y_lm_d2_theta_a, Y_lm_d2_phi_a, Y_lm_d2_thetaphi_a])
    else:
        print('Loading precomputed SH derivatives for strain calculations')
        Y_lm_d1_theta_a, Y_lm_d1_phi_a, Y_lm_d2_theta_a, \
        Y_lm_d2_phi_a, Y_lm_d2_thetaphi_a = np.load(poly_file, \
        allow_pickle = True)
        print('Loading done')
        
    return Y_lm_d1_theta_a, Y_lm_d1_phi_a, Y_lm_d2_theta_a, \
    Y_lm_d2_phi_a, Y_lm_d2_thetaphi_a

def Displacement_strains(A_lm, w_lm, E, v, R, Te, lmax_calc, colat_min = -1e20, \
   colat_max = 1e20, lon_min = -1e20, lon_max = 1e20, only_deflec = False, precomp = True, Y_lm_d1_t = None, Y_lm_d1_p = None, Y_lm_d2_t = None, Y_lm_d2_p = None, Y_lm_d2_tp = None, path = None):
   ...
   if precomp:
      if Y_lm_d1_p is not None:
         ...
         if path is None:
            raise ValueError(
            "Need to speficify the path, here the path is {:s}."
            .format(repr(path)))
         Y_lm_d1_t, Y_lm_d1_p, Y_lm_d2_t, Y_lm_d2_p, \
         Y_lm_d2_tp = SH_deriv_theta_phi(lmax_calc, path)
   else:
      if quiet is False:
         print('Compute Spherical Harmonic derivatives on the' + \
   ...
   for theta in np.linspace(0,180,n, endpoint = False)*pi/180.:
       t_i += 1
       p_i = -1
       if theta < ((colat_min-1)*pi/180.) or theta > ((colat_max+1)*pi/180.):
       ...
       for phi in np.linspace(0,360,2*n,endpoint = False)*pi/180.:
           p_i += 1
           if phi < ((lon_min-1)*pi/180.) or phi > ((lon_max+1)*pi/180.):
               # Not in the lat / lon range we investigate.
               continue

           if precomp:
               y_lm = pysh.expand.spharm(lmax_calc, theta, phi, \
                degrees = False)
               ...
               Y_lm_d1_tb, Y_lm_d1_pb, Y_lm_d2_tb, Y_lm_d2_pb,\
                Y_lm_d2_tpb, y_lm = SH_deriv(theta,phi,lmax_calc)
               w_lm_ylm = np.sum(w_lm * y_lm)
           ...
           eps_phi[t_i,p_i] = R_m1 * (d2Aphi * csc2 + \
           d1Atheta * cot + w_lm_ylm)
           omega[t_i,p_i] = R_m1 * csc * (d2Athetaphi - \
           cot * d1Aphi)
           # omega[t_i,p_i] = 2. * R_m1 * csc * (d2Athetaphi -
           # cot * d1Aphi) Error in Banerdt
           
           kappa_theta[t_i,p_i] = -R_m1**2 * (d2wtheta + \
           w_lm_ylm)
           kappa_phi[t_i,p_i] = -R_m1**2 * (d2wphi * csc2 + \
           d1wtheta * cot + w_lm_ylm)
           tau[t_i,p_i] = -R_m1**2 * csc * (d2wthetaphi - \
           cot * d1wphi)
           # tau[t_i,p_i] = - 2. * R_m1**2 * csc * (d2wthetaphi -
           # cot * d1wphi) Error in Banerdt
           
           stress_theta[t_i,p_i] = eps_theta[t_i,p_i] + v * \
           eps_phi[t_i,p_i] + Te/2. * (kappa_theta[t_i,p_i] + \
           v * kappa_phi[t_i,p_i])
           stress_phi[t_i,p_i] = eps_phi[t_i,p_i] + v * \
           eps_theta[t_i,p_i] + Te/2. * (kappa_phi[t_i,p_i] + \
           v * kappa_theta[t_i,p_i])
           stress_theta_phi[t_i,p_i] = tau[t_i,p_i]

   stress_theta *= DpsiTeR/1e6 # MPa
   stress_phi *= DpsiTeR/1e6 # MPa
   stress_theta_phi *= (0.5 * DpsiTeR * (1.-v) * Te/2.) / 1e6 # MPa
   
   kappa_theta *= Te/2.
   kappa_phi *= Te/2.
   tau *= Te/2.

   return stress_theta, stress_phi, \
   stress_theta_phi, eps_theta, eps_phi, omega, \
   kappa_theta, kappa_phi, tau

def Principal_strain_angle(eps_theta, eps_phi, eps_theta_phi, \
   k_theta, k_phi, k_theta_phi):
   
   strain_theta = -eps_theta - k_theta
   strain_phi = -eps_phi - k_phi
   strain_theta_phi = -eps_theta_phi - k_theta_phi
   min_strain = 0.5 * (strain_theta + strain_phi - np.sqrt((strain_theta-strain_phi)**2 + 4*strain_theta_phi**2))
   max_strain = 0.5 * (strain_theta + strain_phi + np.sqrt((strain_theta-strain_phi)**2 + 4*strain_theta_phi**2))
   sum_strain = min_strain + max_strain

   principal_angle = 0.5 * np.arctan2(2*strain_theta_phi, strain_theta-strain_phi) * 180./np.pi
   principal_angle2 = 0.5 * np.arctan2(2*strain_theta_phi, strain_phi-strain_theta) * 180./np.pi
